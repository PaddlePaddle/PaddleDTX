...
# PaddleDTX service contract name
...
function start() {
  # 1. Standardized service config.toml
  # 1. 标准化配置文件
  standardizedConf
  ...
  # 4. Start decentralized storage service
  # 4. 启动去中心化存储网络
  startXdata
  sleep 5

  # 5. Start PaddleDTX service
  # 5. 启动多方安全计算网络
  startPaddleDTX
}

function stop() {
  # Stop PaddleDTX service
  # 停止多方安全计算网络
  print_blue "==========> Stop PaddleDTX service ..."
  docker-compose -f ../testdata/executor/docker-compose.yml down
  # Stop decentralized storage service
  # 停止去中心化存储网络
  print_blue "==========> Stop Decentralized storage service ..."
  docker-compose -f ../testdata/xdata/docker-compose.yml down
  # Stop xchain network
  # 停止区块链网络
  print_blue "==========> Stop Xchain network ..."
  docker-compose -f ../testdata/blockchain/docker-compose.yml down
  sleep 3
  print_green "==========> PaddleDTX service stopped !"
}


function standardizedConf() {
  ...
  do
    conf=${file%.sample}
    cp -f $file $conf
    sed -i "s/MNEMONIC/$mnemonic/g" $conf
    sed -i "s/CONTRACTNAME/$contractName/g" $conf
    sed -i "s/CONTRACTACCOUNT/$contractAccount/g" $conf
    sed -i "s/EXECUTOR1PRIVATEKEY/$executor1PrivateKey/g" $conf
    sed -i "s/EXECUTOR2PRIVATEKEY/$executor2PrivateKey/g" $conf
    sed -i "s/LEASE1PRIVATEKEY/$executor1PrivateKey/g" $conf
    sed -i "s/LEASE2PRIVATEKEY/$executor2PrivateKey/g" $conf
    sed -i "s/FILESNAMESPACES/$namespaces/g" $conf
  done
}
...
function startXchain() {
  ...
  print_green "==========> Xchain network start success !"
}

# startXdata start Decentralized storage service with docker compose
# 通过docker-compose启动去中心化存储网络
function startXdata() {
  print_blue "==========> Decentralized storage service start ..."
  docker-compose -f ../testdata/xdata/docker-compose.yml up -d
  sleep 6

  xdataContainers="dataowner1.node.com dataowner2.node.com storage1.node.com storage2.node.com storage3.node.com"
  checkContainerStatus "$xdataContainers" "Decentralized storage service"
  print_green "==========> Decentralized storage service start success !"
}

# startPaddleDTX start paddleDTX service with docker compose
# 通过docker-compose启动多网安全计算网络
function startPaddleDTX() {
  print_blue "==========> PaddleDTX service start ..."
  ...
  print_green "========================================================="
  print_green "          PaddleDTX service start success !            "
  print_green "========================================================="
}
...
function compileContract() {
  ...
  # Copy contract file to xchain service container
...
function installContract() {
  print_blue "==========> InstallContract start ..."
  compileContract
  ...
  " | grep "The gas you cousume is" | awk -F: '{print $2}'| sed -e 's/\r//g' | sed 's/ //g'`
  ...
  print_green "==========> Install contract success ! "
}

function checkOperateResult() {
  errMessage=`echo "$1" | grep "Error\|Fail\|err"`
  if [ "$errMessage" ]; then
    print_red "start network error: $1"
    exit 1
  fi
}
