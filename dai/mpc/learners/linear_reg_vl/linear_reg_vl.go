...
var (
	logger = logrus.WithField("module", "mpc.learners.linear_reg_vl")
)
...
type Learner struct {
	...
	lEvaluated   bool          // lEvaluated means whether to perform LiveEvaluation
	...
	// that's to say, the process will continue as long as receive specific messages
	stopSigNeglected bool
	// if in `pauseRound`, the process will pause
	pauseRound uint64
}
...
func (l *Learner) advance(message *pbLinearRegVl.Message) (*pb.TrainResponse, error) {
	mType := message.Type
	...
	switch mType {
	case pbLinearRegVl.MessageType_MsgPsiEnc: // local message
		...
		go func() {
			m := &pbLinearRegVl.Message{
				Type: pbLinearRegVl.MessageType_MsgPsiAskReEnc,
	...
	case pbLinearRegVl.MessageType_MsgPsiAskReEnc: // local message
		newMess := &pbLinearRegVl.Message{
			Type:              pbLinearRegVl.MessageType_MsgPsiReEnc,
		...
		if done {
			go func() {
				m := &pbLinearRegVl.Message{
					Type: pbLinearRegVl.MessageType_MsgPsiIntersect,
				}
				l.advance(m)
			}()
		}

	case pbLinearRegVl.MessageType_MsgPsiReEnc:
	...
	case pbLinearRegVl.MessageType_MsgPsiIntersect: // local message
		...
		if done {
			...
			go func() {
				m := &pbLinearRegVl.Message{
					Type: pbLinearRegVl.MessageType_MsgTrainHup,
				}
				l.advance(m)
			}()
		}

	case pbLinearRegVl.MessageType_MsgTrainHup: // local message
		l.procMutex.Lock()
		...
		if learnerStatusEndPSI == l.status {
			...
			m := &pbLinearRegVl.Message{
				Type:       pbLinearRegVl.MessageType_MsgHomoPubkey,
			...
			go func() {
				if !l.lEvaluated {
					m := &pbLinearRegVl.Message{
						Type:      pbLinearRegVl.MessageType_MsgTrainLoop,
						LoopRound: 0, // start Round-0
					}
					l.advance(m)
				} else {
					cbm := &pbLinearRegVl.Message{
						Type:         pbLinearRegVl.MessageType_MsgContinueLoop,
						LoopRound:    0, // to start Round-0
						TriggerRound: l.triggerRound + l.triggerInter,
					}
					fwm := &pbLinearRegVl.Message{
						Type:       pbLinearRegVl.MessageType_MsgTrainSet,
	...
	case pbLinearRegVl.MessageType_MsgTrainSet: // from live evaluator
	...
	case pbLinearRegVl.MessageType_MsgContinueLoop: // from live evaluator
	...
	case pbLinearRegVl.MessageType_MsgHomoPubkey:
	...
	case pbLinearRegVl.MessageType_MsgTrainLoop: // local message
		...
		if newRound == 0 || newRound == l.loopRound+1 {
			...
			go func() {
				m := &pbLinearRegVl.Message{
					Type:      pbLinearRegVl.MessageType_MsgTrainCalLocalGradCost,
					LoopRound: l.loopRound,
				}
				l.advance(m)
			}()
	...
	case pbLinearRegVl.MessageType_MsgTrainCalLocalGradCost: // local message
		loopRound := message.LoopRound
		if loopRound == l.loopRound {
			...
			if t == 1 {
				m := &pbLinearRegVl.Message{
					Type:      pbLinearRegVl.MessageType_MsgTrainPartBytes,
				...
				go func() {
					m := &pbLinearRegVl.Message{
						Type:      pbLinearRegVl.MessageType_MsgTrainCalEncGradCost,
						LoopRound: loopRound,
					}
					l.advance(m)
				}()
	...
	case pbLinearRegVl.MessageType_MsgTrainPartBytes:
		loopRound := message.LoopRound
	...
	case pbLinearRegVl.MessageType_MsgTrainCalEncGradCost: // local message
		loopRound := message.LoopRound
		if loopRound == l.loopRound {
			...
			if t == 1 {
				m := &pbLinearRegVl.Message{
					Type:             pbLinearRegVl.MessageType_MsgTrainEncGradCost,
	...
	case pbLinearRegVl.MessageType_MsgTrainEncGradCost:
		loopRound := message.LoopRound
		...
		if loopRound == l.loopRound {
			...
			if t == 1 {
				go func() {
					m := &pbLinearRegVl.Message{
						Type:      pbLinearRegVl.MessageType_MsgTrainDecLocalGradCost,
						LoopRound: loopRound,
					}
					l.advance(m)
				}()
	...
	case pbLinearRegVl.MessageType_MsgTrainDecLocalGradCost: // local message
		loopRound := message.LoopRound
		if loopRound == l.loopRound {
			...
			if t == 1 {
				m := &pbLinearRegVl.Message{
					Type:      pbLinearRegVl.MessageType_MsgTrainGradAndCost,
	...
	case pbLinearRegVl.MessageType_MsgTrainGradAndCost:
		loopRound := message.LoopRound
		...
		if loopRound == l.loopRound {
			...
			if t == 1 {
				go func() {
					m := &pbLinearRegVl.Message{
						Type:      pbLinearRegVl.MessageType_MsgTrainUpdCostGrad,
						LoopRound: loopRound,
					}
					l.advance(m)
				}()
	...
	case pbLinearRegVl.MessageType_MsgTrainUpdCostGrad: // local message
		loopRound := message.LoopRound
		if loopRound == l.loopRound {
			...
			m := &pbLinearRegVl.Message{
				Type:      pbLinearRegVl.MessageType_MsgTrainStatus,
				Stopped:   stopped,
				LoopRound: loopRound,
			}
			...
			go func() {
				m := &pbLinearRegVl.Message{
					Type:      pbLinearRegVl.MessageType_MsgTrainCheckStatus,
					LoopRound: loopRound,
				}
				l.advance(m)
			}()
	...
	case pbLinearRegVl.MessageType_MsgTrainStatus:
		loopRound := message.LoopRound
	...
	case pbLinearRegVl.MessageType_MsgTrainCheckStatus: // local message
		loopRound := message.LoopRound
		...
		if decided {
			if l.stopSigNeglected {
				...
				go func() {
					m := &pbLinearRegVl.Message{
						Type:      pbLinearRegVl.MessageType_MsgCheckPauseRound,
						LoopRound: loopRound,
					}
					l.advance(m)
				}()
				...
				logger.WithFields(logrus.Fields{
					"address":   l.address,
					"loopRound": l.loopRound,
				}).Infof("learner[%s] trained out a model this round[%d], got ready to stop.", l.id, loopRound)
				go func() {
					m := &pbLinearRegVl.Message{
						Type:      pbLinearRegVl.MessageType_MsgTrainModels,
						LoopRound: loopRound,
					}
					l.advance(m)
				}()
				...
				go func() {
					if !l.lEvaluated {
						...
						logger.WithFields(logrus.Fields{
							"address":   l.address,
							"loopRound": l.loopRound,
						}).Infof("learner[%s] did not train out model this round[%d], and got ready to start new round[%d].", l.id, loopRound, loopRound+1)
						m := &pbLinearRegVl.Message{
							Type:      pbLinearRegVl.MessageType_MsgTrainLoop,
						...
						if loopRound == l.triggerRound {
							logger.WithFields(logrus.Fields{
								"address":   l.address,
								"loopRound": l.loopRound,
							}).Infof("learner[%s] did not train out model this round[%d], but reached trigger round[%d].", l.id, loopRound, l.triggerRound)
							cbm := &pbLinearRegVl.Message{
								Type:         pbLinearRegVl.MessageType_MsgContinueLoop,
								LoopRound:    loopRound,
								TriggerRound: l.triggerRound + l.triggerInter,
							...
							logger.WithFields(logrus.Fields{
								"address":   l.address,
								"loopRound": l.loopRound,
							}).Infof("learner[%s] did not train out model this round[%d], got ready to start new round[%d].", l.id, loopRound, loopRound+1)
							m := &pbLinearRegVl.Message{
								Type:      pbLinearRegVl.MessageType_MsgTrainLoop,
								LoopRound: loopRound + 1, //for starting new round
							}
	...
	case pbLinearRegVl.MessageType_MsgTrainModels: // local message
		l.procMutex.Lock()
		defer l.procMutex.Unlock()
	...
	case pbLinearRegVl.MessageType_MsgCheckPauseRound: // local message
		loopRound := message.LoopRound
...
func (l *Learner) triggerLiveEvaluation(msgType pb.TriggerMsgType, callbackMsg *pbLinearRegVl.Message, forward *pbLinearRegVl.Message) error {
	...
	if err != nil {
		logger.WithFields(logrus.Fields{
			"address":   l.address,
			"loopRound": l.loopRound,
		}).Warnf("failed to trigger evaluation: %s", err.Error())
		return err
	}
...
// retries 2 times at most
func (l *Learner) sendMessageWithRetry(message *pbLinearRegVl.Message, address string) (*pbLinearRegVl.Message, error) {
	times := 3

	var m *pbLinearRegVl.Message
	var err error
	for i := 0; i < times; i++ {
		if i > 0 {
...
func (l *Learner) sendMessage(message *pbLinearRegVl.Message, address string) (*pbLinearRegVl.Message, error) {
...
// NewLearner returns a VerticalLinearRegression Learner
...
func NewLearner(id string, address string, params *pbCom.TrainParams, samplesFile []byte,
	...
	go func() {
		m := &pbLinearRegVl.Message{
			Type: pbLinearRegVl.MessageType_MsgPsiEnc,
		}
		l.advance(m)
	}()
	return l, nil
...
// NewLearner returns a VerticalLinearRegression Learner but doesn't run it
// id is the assigned id for Learner
// address indicates local mpc-node
// parties are other learners who participates in MPC, assigned with mpc-node address usually
// rpc is used to request remote mpc-node
