...
var (
	logger = logrus.WithField("module", "mpc.models.linear_reg_vl")
)
...
func (model *Model) Advance(payload []byte) (*pb.PredictResponse, error) {
	m := &pbLinearRegVl.PredictMessage{}
...
func (model *Model) advance(message *pbLinearRegVl.PredictMessage) (*pb.PredictResponse, error) {
	mType := message.Type
	...
	case pbLinearRegVl.MessageType_MsgPsiAskReEnc: // local message
		newMess := &pbLinearRegVl.PredictMessage{
	...
	case pbLinearRegVl.MessageType_MsgPsiReEnc:
		...
		retM := &pbLinearRegVl.PredictMessage{
			Type: pbLinearRegVl.MessageType_MsgPsiReEnc,
			To:   message.From,
			From: model.address,
	...
	case pbLinearRegVl.MessageType_MsgPsiIntersect: // local message
		...
		if done {
			...
			go func() {
				m := &pbLinearRegVl.PredictMessage{
					Type: pbLinearRegVl.MessageType_MsgPredictHup,
				}
				model.advance(m)
			}()
		}

	case pbLinearRegVl.MessageType_MsgPredictHup: // local message
		...
		if modelStatusEndPSI == model.status {
			...
			if !model.params.IsTagPart {
				...
				newMess := &pbLinearRegVl.PredictMessage{
					Type:        pbLinearRegVl.MessageType_MsgPredictPart,
			...
			go func() {
				m := &pbLinearRegVl.PredictMessage{
					Type: pbLinearRegVl.MessageType_MsgPredictSum,
				}
				model.advance(m)
			}()
		}

	case pbLinearRegVl.MessageType_MsgPredictPart:
	...
	case pbLinearRegVl.MessageType_MsgPredictSum: // local message
		model.procMutex.Lock()
		defer model.procMutex.Unlock()
		...
		if modelStatusStartPredict == model.status {
			if !model.params.IsTagPart {
				...
				done, outcomes := model.deStandardizeOutput()
				if done {
					model.status = modelStatusEndPredict
					...
					go func() {
						logger.WithField("IsTagPart", model.params.IsTagPart).Infof("model[%s] finished prediction and outcomes are[%v].", model.id, outcomes)
						model.rh.SaveResult(&pbCom.PredictTaskResult{
							TaskID:   model.id,
							Success:  true,
							Outcomes: outs,
...
// retries 2 times at most
func (model *Model) sendMessageWithRetry(message *pbLinearRegVl.PredictMessage, address string) (*pbLinearRegVl.PredictMessage, error) {
	times := 3

	var m *pbLinearRegVl.PredictMessage
	var err error
	for i := 0; i < times; i++ {
		if i > 0 {
...
func (model *Model) sendMessage(message *pbLinearRegVl.PredictMessage, address string) (*pbLinearRegVl.PredictMessage, error) {
