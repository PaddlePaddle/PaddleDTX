...
import (
	...
	"github.com/PaddlePaddle/PaddleDTX/xdb/pkgs/crypto"
)
...
func MarshalPrivateKey(privkey *ecdsa.PrivateKey) PrivateKey {
	var res PrivateKey
	copy(res[:], crypto.PadStart(privkey.D.Bytes(), 32))
	return res
}

// MarshalPublicKey marshal public key to local types
func MarshalPublicKey(pubkey *ecdsa.PublicKey) PublicKey {
	x := crypto.PadStart(pubkey.X.Bytes(), 32)
	y := crypto.PadStart(pubkey.Y.Bytes(), 32)
	var res PublicKey
	copy(res[:], x)
	copy(res[32:], y)
...
// PublicKeyFromPrivateKey suger func
...
func Sign(privkey PrivateKey, digest []byte) (Signature, error) {
	...
	rr, ss := crypto.PadStart(r.Bytes(), 32), crypto.PadStart(s.Bytes(), 32)
	var sig Signature
	copy(sig[:], rr)
	copy(sig[32:], ss)
	return sig, nil
...
func DecodePrivateKeyFromString(s string) (PrivateKey, error) {
	...
	if err != nil {
		return PrivateKey{}, errorx.NewCode(err, errorx.ErrCodeParam, "invalid format")
	}
	if len(bs) != PrivateKeyLength {
		return PrivateKey{}, errorx.New(errorx.ErrCodeParam, "invalid length")
	}
	copy(privateKey[:], bs)
...
func DecodePublicKeyFromString(s string) (PublicKey, error) {
	...
	bs, err := hex.DecodeString(s)
	if err != nil {
		return PublicKey{}, errorx.NewCode(err, errorx.ErrCodeParam, "invalid format")
	}
	if len(bs) != PublicKeyLength {
		return PublicKey{}, errorx.New(errorx.ErrCodeParam, "invalid length")
	}
	copy(publicKey[:], bs)
	if _, err := ParsePublicKey(publicKey); err != nil {
		return PublicKey{}, errorx.New(errorx.ErrCodeParam, "invalid ecdsa public key")
	}
...
func DecodeSignatureFromString(s string) (Signature, error) {
	...
	bs, err := hex.DecodeString(s)
	if err != nil {
		return Signature{}, errorx.NewCode(err, errorx.ErrCodeParam, "invalid format")
	}
	if len(bs) != PublicKeyLength {
		return Signature{}, errorx.New(errorx.ErrCodeParam, "invalid length")
	}
	copy(sig[:], bs)
	return sig, nil
}
