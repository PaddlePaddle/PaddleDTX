...
// failedCmd gets failed challenge by filters
var failedCmd = &cobra.Command{
	Use:   "failed",
	Short: "get failed challenges by filters",
	Run: func(cmd *cobra.Command, args []string) {
		...
		fmt.Printf("Failed challenges from %s to %s\nNum: %d\n\n", start, end, len(challenges))
	},
}
...
func init() {
	...
	failedCmd.Flags().StringVarP(&owner, "owner", "o", "", "file owner")
	failedCmd.Flags().StringVarP(&storageNode, "node", "n", "", "storage node")
	...
	failedCmd.Flags().Int64VarP(&limit, "limit", "l", blockchain.ListMaxNumber, "limit")
